...
export const SYNC_EXIT_CONTRACT_INTERFACE = new utils.Interface(
  require('../abi/SyncExit.json').abi
);
export const ZKL_CONTRACT_INTERFACE = new utils.Interface(
  require('../abi/ZKL.json').abi
);
...
export const MULTICALL_INTERFACE = new utils.Interface(
...
export function serializeOrder(order: OrderData): Uint8Array {
  ...
  const priceBytes = bigintToBytesBE(
    BigNumber.from(order.price).toBigInt(),
    15
  );
...
export async function serializeOrderMatching(
  ...
  const expectQuoteAmountBytes = serializeAmountFull(
    matching.expectQuoteAmount
  );
  return ethers.utils.concat([
    type,
    accountIdBytes,
