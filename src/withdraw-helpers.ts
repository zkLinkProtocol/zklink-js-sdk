...
Wallet.prototype.withdrawPendingBalance = async function (
  ...
  const withdrawAmount = amount
    ? amount
    : await zksyncContract.getPendingBalance(from, tokenAddress)
...
Wallet.prototype.withdrawPendingBalances = async function (
  ...
  const multicallAddress =
    multicallParams.address || getMulticallAddressByNetwork(multicallParams.network)

  const zksyncContract = await this.getZkSyncMainContract(linkChainId)
  const gasPrice = await this.ethSigner.getGasPrice()

  const tokensAddresses = tokens.map((token) =>
    this.provider.tokenSet.resolveTokenAddress(token, linkChainId)
  )
